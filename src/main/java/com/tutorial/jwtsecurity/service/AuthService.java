...
public class AuthService {
    ...
    @Transactional
    public ResponseDto<?> signup(MemberSignupDto memberSignupDto) {
        if (memberRepository.existsByNickname(memberSignupDto.getNickname())) {
            throw new RuntimeException("이미 가입되어 있는 유저입니다");
        }
        // DB에 해당 닉네임을 가진 회원 없을 때
    ...
    public ResponseDto<?> login(MemberRequestDto memberRequestDto, HttpServletResponse response) {
        ...
        // HTTP 응답의 header 에 토큰 정보 볼 수 있도록
        response.addHeader("Access-Token", tokenDto.getAccessToken());  // name 부분은 정하기 나름
